...
This code snippet creates a new data-frame with 3 columns (`make`, `mpg`, `cylinders`) and 6 rows.
...
Using the colSums function, we can return the count of values equal to 0 in each column.
...
We can check the class of the variable again and see that it now belongs to the factor class.
...
In this first graph, I am going to plot the gender information of stopped drivers for the whole time frame:

*Note:* I used this [StackOverflow](https://stackoverflow.com/questions/11610377/how-do-i-change-the-formatting-of-numbers-on-an-axis-with-ggplot) post to change the y-axis value formatting.
...
g = ggplot(data = group.gender.total, aes(x = Gender, y = Count)) +
    geom_bar(stat = "identity", fill = "darkblue", width = 0.5) +
    xlab("Gender") + 
    ylab("Number of Stops") + 
    ggtitle("Total Traffic Stops by Gender from January 2007 to June 2016") + 
    scale_y_continuous(labels = scales::comma)

g
```

At first stage, I grouped the data by gender information and renamed the columns (this is not a necessary step). Following this, I plotted the data using ggplot.
...
*Note:* I had to add the `stat = 'identity'` part to the plotting as I was getting the `` `stat_count()` must only have an x or y aesthetic. `` error. This is the [StackOverflow](https://stackoverflow.com/questions/61068031/error-stat-count-can-only-have-an-x-or-y-aesthetic) post I found the solution.
...
g = ggplot(data = group.gender.year.total, aes(x = factor(Year), y = Count, fill = Gender)) +
    geom_bar(stat="identity", position = "dodge") +
    xlab('Year') + 
    ylab('Number of Stops') + 
    ggtitle('Total Annual Traffic Stops by Gender') +
    scale_y_continuous(labels = scales::comma)
